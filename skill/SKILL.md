...
2. 如果改了 runtime 语义，同时检查并更新对应 runtime 测试。
...
6. 始终只保留一条主路径，不要为了局部方便再发明并行 runtime 概念。

## 硬规则

- 把 `VNode` 当成真实 runtime DOM 节点看待。`VNode.id` 是唯一 runtime identity。
...
- 生产事件分发依赖 runtime node `id`。不要在 service bridge 里补 `ui-id` fallback 路由。
...
- 公开 GUI 能力保持 path-first：`query / trigger` 面向 path。runtime `id` 留在 bridge 内部和白盒测试 helper。
...
- `h_map` item root 即使返回 `Dyn(...)`，当前也仍按稳定 runtime fragment 处理。
...
- 公开 GUI 能力应保持 path-first 的 `query / trigger`。runtime `id` 仍是内部细节。
- 业务测试目标应站在高层 `query / trigger` 语义上。runtime `id` 只给 bridge 白盒 helper。
