...
7. 出现 现有语义做不到 或 实现不正确 的情形，先和用户报告，不要在错误的模型上写错误的东西
...
- 列表查询的推荐形态是 `<list-ui-id>/0/<inner-ui-id>`（0 已经是一个名称空间，不要在里面再建额外的名称空间，直接访问内层 ui-id 即可）
- `<list-ui-id>/0` 可以访问到列表项 0 里的第一个根节点（如果列表项是数组的情形），但这不是稳定位置，会随dom结构变化而变化
- 同理 `<list-ui-id>` 相当于 `<list-ui-id>/0`，出于同样理由不推荐依赖这个路径
