...
`scope` 不是用户日常手工标记的东西，尤其 `comp` 的 root 不自己开 scope。一个节点有没有 name scope，取决于它是不是外部赋予的挂载点。像 host 这种拥有自己子实例的宿主，声明一个挂载点来挂别的 entry，这个挂载点携带的 `ui-id` 就是 name scope 入口, 它需要带上 scope=Scope 参数。
...
`h_map / h_map_dyn` 的重点是稳定 item 和 list 查询语义，不额外强调固定 DOM 包裹层。

### `ui-id`

这是公开可寻址语义的根。path 的稳定性从这里开始。

`ui-id` 不是运行时节点 identity，也不是业务序号槽位。它负责给外部世界一个稳定名字，同时默认也作为样式锚点。

`ui-id` 的语义层级是全局定义，不是实例 data。因为它会抛错，实际求值时机放在 init 闭包里。

默认推荐短 `ui-id`。path 语义靠名称空间和列表作用域补全，不靠在单个 `ui-id` 里重复编码父层语义。

普通 `ui-id` 表示稳定名字。name scope 来自挂载点，list scope 来自动态列表主路径。

### 两种 query scope

这层至少要分清两种 scope：

- 名称空间 scope：由挂载点提供，用来缩短局部 path
- 列表 scope：由动态列表主路径提供，用来解析索引和 item 内路径
...
- `Entry` 定义一类可实例化对象：数据、session、runtime、view、actions
- `Instance` 是运行时实例，可以被创建、挂树、查询、关闭、调用 action

这里要分清三层实例状态：

- `data`：同类 entry 之间可共享，可序列化
- `session`：实例独立，可序列化
- `runtime`：实例独立，不可序列化，每次临时创建

`ui-id` 不属于这三层实例业务态，它是定义层语义，只是创建时机被拖到 init 闭包里。
...
- **禁止再写为特定类型而存在的json helper，要写就必须是泛型的，如果在代码里看到此类helper，向用户报告来组织清除工作**
...
- 查 `ui-id`、query path、scope、list scope、`h_map`、query 合同、测试 path 时，读 `references/dom-query.md`
- 查 `DomCmd`、VNode 生命周期、挂载/移除、样式节点、bridge 和 mock 看到的 DOM 形状时，读 `references/dom.md`
- 查 websocket / http / CLI `--json` 协议、`Json` codec、DTO、边界编解码，或怀疑内部 typed 语义被 `Json` 污染时，读 `references/json.md`
- 如果任务同时碰到多条线，就按“最接近当前改动边界”的 reference 先读，不要只停在 `SKILL.md`
...
- `style` 和 `ui-id` 强绑定；`ui-id` 会生成全局唯一 class，`style` 默认就是精准作用到这个 ui 位置
- 默认推荐短 `ui-id`
- `/` 不要出现在 `ui-id` 里
...
- `comp` 的 root 默认吃外面挂载点给的 scope，不要自己再手工标一个 root scope
...
- 动态样式优先挂在对应 `ui-id` 的 `style` 上
- 不要为同一个锚点机械重复写一份同义 `class`
- `ui-id` 会派生全局唯一 class，`style` 默认直接绑定这个唯一 class，不要再靠宽泛 DOM 选择器猜位置
- 样式语义默认跟着 `ui-id` 位置走，不按 DOM 包裹层和标签名机械抄长路径
