# MetaEditor DOM Runtime 语义

这份参考文档描述当前 runtime 模型，以及写 DOM/UI/bridge 代码时必须遵守的语义边界。

## 1. 核心心智模型

`VNode` 应被视为 runtime 里的真实 DOM 节点。

- `VNode.id` 是唯一 runtime identity
- 节点可以移动并继续接收更新
- 节点只应在真的消失时被移除
...
- 保留 item 的 listener 会继续挂在原来的 runtime root 上
- 被移除的 item 会停止它内部的动态更新
- item root 即使返回 `Dyn(...)`，当前也会保留稳定 runtime fragment identity
...
bridge 与 runtime 必须同步。

当前稳定规则：

- `DomCmd` 编号应集中定义
- bridge 消息类型应集中定义
- 生产事件分发依赖 runtime node `id`
- 公开 GUI 能力保持 path-first 的 `query / trigger`
- runtime `id` 留在 bridge 内部和白盒测试 helper
