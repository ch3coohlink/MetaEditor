...
这轮还把 `h_map` 的输入约束重新收紧了一遍：前面已经确认过，`h_map` 先只能稳定支持函数返回数组这一路，其他的数据容器暂时不考虑。这轮是想恢复 `IntentDoc` 里那种 `h.map` 既能吃普通数组、也能吃动态函数的输入形态，才试着把函数类型和泛型 trait 拼成一个统一入口；但后面再试 trait 方案时，MoonBit 又在泛型 trait 和函数类型的组合上卡住了语法和类型系统限制，编不过也落不稳，所以当前先不再往里加那层抽象。`h_map` 现在统一保留成“函数返回数组”的单一路径，像 `fn() { items.get() }` 这种输入继续作为当前可编译、可监听的边界。trait 风格的多输入入口先不做，避免把已经跑通的渲染链拆出一条并行语义。
