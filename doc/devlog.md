...
这一轮把 `h_map` 继续收成了带缓存的薄 helper：它只针对 `Array` 做动态列表生成，子项的构造结果和 index 读取器会按 item 复用，重复出现的 item 不会再次走 child 构造。回归里也专门补了计数断言，确认列表从 `["A", "B"]` 变成 `["B", "C"]` 时，缓存命中的 `B` 没有重复构造，当前 `moon test` 还是全部通过。
