...
## 4. 工程结构 (Engineering Structure)

```text
MetaEditor/
├── src/                # 核心运行时 (MoonBit/Wasm in Browser Worker)
│   ├── reactive.mbt    # 响应式系统 (Signals, Computed, Watch)
│   ├── dom.mbt         # DOM 指令生成与宿主测量代理
│   ├── op.mbt          # 原子操作 (Ops), 补丁校验 (CAS) 与 Undo/Redo
│   └── bridge.js       # 宿主桥接 (Main Thread JS): DOM 执行, 布局测量, 动画插值
├── cli/                # 外部设施: Node.js 服务端与 CLI 控制端工具
├── app/                # 应用层: 基于框架构建的具体编辑器业务逻辑
├── test/                # 测试层: 单元测试、结构化几何断言、浏览器运行时自动化测试
└── index.html          # 宿主页面入口
```
